...
import { Loader2, Lock, Mail, Briefcase, Eye, EyeOff, Copy, CheckCheck, Key, ReceiptText, UtensilsCrossed, ArrowLeft, ArrowRight } from 'lucide-react'
...
export default function SignupForm() {
	...
	async function onSubmit() {
		...
		try {
			const response = await fetch('/api/auth/signup', {
				...
				body: JSON.stringify({ name, email, password, businessType: 'restaurant', trackingMode })
			})
	...
	return (
			...
			<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
				<div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 space-y-5">
					...
					<div className="rounded-xl bg-red-50 border border-red-200 p-4 text-sm text-red-700 space-y-1">
						<p className="font-semibold">âš  Important â€” store this somewhere safe:</p>
					...
					<button type="button" onClick={() => router.push('/login?registered=true')}
						...
						I've Saved My Key â€” Continue to Login
					</button>
				</div>
			</div>
		)}
		...
		{/* â”€â”€ Step 2: Tracking mode picker â”€â”€ */}
		{step === 2 && (
			<div className="space-y-4">
				...
				<div>
					...
					<p className="text-sm font-semibold text-gray-800">How do you want to track your restaurant?</p>
