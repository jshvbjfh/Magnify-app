...
import { Save, CheckCircle2, FileText, ReceiptText, UtensilsCrossed, Layers, Cloud, RefreshCw, Download, Upload, ShieldCheck, ChevronDown, Briefcase, AlertTriangle, Clock, Printer, Boxes, BedDouble } from 'lucide-react'
...
export default function RestaurantSettings() {
    ...
    Promise.all([
      ...
      .then(([setupData, profileData, serverData, syncStatusData, conflictData, syncSecretData]) => {
        ...
        if (setupData) {
          ...
          setHotelEnabled(setupData.restaurant?.hotelEnabled === true)
  ...
  async function save() {
    ...
    try {
      ...
      const body: Record<string, unknown> = { name: restaurantName, billHeader, billPrinterIp: billPrinterIp.trim() || null, billPrinterPort: billPrinterPort.trim() ? parseInt(billPrinterPort) || 9100 : null, qrOrderingMode, shiftsEnabled, printPaymentConfirmation, operatingEquipmentEnabled, hotelEnabled }
      ...
      if (savedRestaurant) {
        ...
        setHotelEnabled(savedRestaurant.hotelEnabled === true)
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">For venues that let rooms as well as serve food.</p>
        </div>

        <button type="button" onClick={() => setHotelEnabled(!hotelEnabled)}
          className={`w-full text-left rounded-xl border-2 p-4 transition-all ${
            hotelEnabled ? 'border-orange-500 bg-orange-50' : 'border-gray-200 bg-white hover:border-gray-300'
          }`}>
          <div className="flex items-start gap-3">
            <div className={`p-2 rounded-lg flex-shrink-0 ${hotelEnabled ? 'bg-orange-100' : 'bg-gray-100'}`}>
              <BedDouble className={`h-5 w-5 ${hotelEnabled ? 'text-orange-600' : 'text-gray-500'}`} />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <p className={`text-sm font-bold ${hotelEnabled ? 'text-orange-700' : 'text-gray-800'}`}>
                  Show the Rooms tab
                </p>
                {hotelEnabled && <span className="text-[10px] font-bold bg-orange-500 text-white px-2 py-0.5 rounded-full">On</span>}
              </div>
              <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                Build your rooms the same way you build tables, and reception can see at a glance which are let, which need cleaning and which are out of use. This is a room list, not a booking system — there are no reservations or rate calendars. Switching it off later only hides the tab; every room stays put.
              </p>
            </div>
          </div>
        </button>
