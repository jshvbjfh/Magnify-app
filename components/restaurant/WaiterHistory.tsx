...
import { History, ShoppingBag, ArrowLeftRight, Sparkles, RefreshCw } from 'lucide-react'
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
        {(['today', 'week', 'month'] as const).map(p => (
          <button key={p} onClick={() => setPeriod(p)} className={period === p ? 'px-4 py-1.5 rounded-lg text-sm font-medium bg-white shadow text-gray-900 capitalize' : 'px-4 py-1.5 rounded-lg text-sm font-medium text-gray-500 hover:text-gray-700 capitalize'}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm text-center">
          ...
          <p className="text-lg font-bold text-orange-600 mt-1">{fmtRWF(totalSales)} RWF</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm text-center col-span-2 sm:col-span-1">
          ...
          <p className="text-lg font-bold text-gray-900 mt-1">
            {filteredSales.length ? fmtRWF(totalSales / filteredSales.length) : '0'} RWF
          </p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {[...filteredSales].sort((a, b) => new Date(b.saleDate).getTime() - new Date(a.saleDate).getTime()).map(s => (
        ...
        filteredTxns.length === 0 ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {[...filteredTxns].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(t => (
