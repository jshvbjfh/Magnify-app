...
import { ChefHat, RefreshCw, Plus, X, AlertTriangle, Sparkles, CircleHelp, UtensilsCrossed, Clock } from 'lucide-react'
...
export default function RestaurantKitchen({ onAskJesse }: { onAskJesse?: () => void }) {
  ...
  const [tab, setTab] = useState<'waste' | 'orders'>('waste')
  ...
  async function loadWaste() {
    ...
    try {
      ...
      const nextPendingWaste: PendingWasteItem[] = []
  ...
  const loadKds = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setKdsItems(Array.isArray(data) ? data : [])
      }
    } catch {}
  ...
  useEffect(() => {
    loadKds()
    const id = window.setInterval(loadKds, 5000)
  ...
  async function logWaste(e:React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setSaveError(payload?.error || 'Failed to log waste.'); return }
      setShowForm(false)
  ...
  async function updateKdsStatus(itemId: string, status: 'in_kitchen' | 'ready') {
    setUpdatingId(itemId)
    try {
      const res = await fetch(`/api/restaurant/pending/${itemId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      if (res.ok) {
        setKdsItems(prev => prev.map(it => it.id === itemId ? { ...it, status } : it))
      }
    } catch {}
  ...
  // Group KDS items by order
  const kdsOrders = kdsItems.reduce<Record<string, { orderId: string; orderNumber: string; tableName: string; items: KdsItem[] }>>((acc, item) => {
    if (!acc[item.orderId]) acc[item.orderId] = { orderId: item.orderId, orderNumber: item.orderNumber, tableName: item.tableName, items: [] }
    acc[item.orderId].items.push(item)
    return acc
  }, {})
  const kdsOrderList = Object.values(kdsOrders)
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <h2 className="text-lg font-bold text-gray-800">Kitchen</h2>
          <BranchBadge />
        </div>
        <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
          <button
            onClick={() => setTab('waste')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${tab === 'waste' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
            <AlertTriangle className="h-3.5 w-3.5" />
            Waste
            {pendingWaste.length > 0 && <span className="min-w-4 rounded-full bg-red-500 px-1.5 text-center text-[10px] font-bold text-white">{pendingWaste.length}</span>}
          </button>
          <button
            onClick={() => setTab('orders')}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${tab === 'orders' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
            <UtensilsCrossed className="h-3.5 w-3.5" />
            Orders
            {kdsItems.filter(i => i.status !== 'ready').length > 0 && (
              <span className="min-w-4 rounded-full bg-orange-500 px-1.5 text-center text-[10px] font-bold text-white">
                {kdsItems.filter(i => i.status !== 'ready').length}
              </span>
            )}
          </button>
        </div>
        {tab === 'waste' && (
          <div className="flex items-center gap-2">
            <button onClick={loadWaste} className="p-2 rounded-lg hover:bg-gray-100"><RefreshCw className={loading ? 'h-4 w-4 text-gray-400 animate-spin' : 'h-4 w-4 text-gray-400'} /></button>
        ...
        {tab === 'orders' && (
          <button onClick={loadKds} className="p-2 rounded-lg hover:bg-gray-100"><RefreshCw className={kdsLoading ? 'h-4 w-4 text-gray-400 animate-spin' : 'h-4 w-4 text-gray-400'} /></button>
        )}
      </div>
      ...
      {tab === 'waste' && (
        <>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            Total waste logged: <span className="font-semibold text-red-600">{totalWasteCost.toLocaleString('en-RW', { maximumFractionDigits: 0 })} RWF</span>
          </div>
      ...
      {tab === 'orders' && (
        <>
          {kdsLoading && kdsOrderList.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-10 text-center text-gray-400">Loading orders...</div>
          ) : kdsOrderList.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-10 text-center">
              <UtensilsCrossed className="h-10 w-10 text-gray-300 mx-auto mb-3" />
              <p className="font-medium text-gray-600">No active orders</p>
              <p className="text-sm text-gray-400 mt-1">Pending orders will appear here for kitchen tracking.</p>
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {kdsOrderList.map(order => {
                const allReady = order.items.every(it => it.status === 'ready')
                return (
                  <div key={order.orderId} className={`rounded-xl border shadow-sm overflow-hidden transition-colors ${allReady ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-white'}`}>
                    <div className={`px-4 py-2.5 flex items-center justify-between ${allReady ? 'bg-green-100' : 'bg-gray-50 border-b border-gray-100'}`}>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-gray-900">{order.tableName}</span>
                        <span className="text-xs text-gray-500">{order.orderNumber}</span>
                      </div>
                      <div className="flex items-center gap-1 text-xs text-gray-400">
                        <Clock className="h-3 w-3" />
                        {timeSince(order.items[0]?.addedAt ?? new Date().toISOString())}
                      </div>
                    </div>
                      ...
                      {order.items.map(item => (
                        <div key={item.id} className="px-4 py-3 flex items-center justify-between gap-3">
                          ...
                          <div className="flex-shrink-0">
                            {item.status === 'new' && (
                              <button
                                onClick={() => updateKdsStatus(item.id, 'in_kitchen')}
                              ...
                              >
                                {updatingId === item.id ? '...' : 'Start cooking'}
                              </button>
                            )}
                            {item.status === 'in_kitchen' && (
                              <button
                                onClick={() => updateKdsStatus(item.id, 'ready')}
                                disabled={updatingId === item.id}
                                className="rounded-lg bg-blue-500 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-blue-600 disabled:opacity-50"
                              >
                                {updatingId === item.id ? '...' : 'Mark ready'}
                              </button>
                            )}
                            {item.status === 'ready' && (
